...
const listingSchema = {
  ...
  properties: {
    ...
    listing: {
      ...
      properties: { descriptionMarkdown: { type: "string" } },
    },
  },
};
  ...
  return [
    {
      ...
      content: [
        ...
        "Do not invent features, versions, pricing, compatibility, APK URLs, package names, screenshots, release notes, or changelogs.",
        "Do not include developer build instructions, contribution notes, license text, badges, raw tables, or API-key setup unless necessary for an end user.",
      ...
      content: JSON.stringify({
        ...
        outputRules: {
          ...
          listingDescriptionMarkdown: [
            ...
            "Add short sections only when supported by the README, such as Key features, How it works, Controls, Requirements, or Notes.",
...
async function callOpenRouter({ model, messages }) {
  ...
  const response = await fetch(openRouterUrl, {
    ...
    body: JSON.stringify({
      ...
      response_format: {
        type: "json_schema",
        json_schema: { name: "rokidbrew_store_listing", strict: true, schema: listingSchema },
      },
    }),
  });
...
async function main() {
  ...
  const updated = pickStoreFields(app, { ...generated, ...(args.noReleases ? {} : { releases }) });
  if (repo) {
